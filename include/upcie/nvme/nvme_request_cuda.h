 ...
 * registry.
 *
 * The buffer is resolved page-by-page through the registry's phys_lut, with
 * no contiguity assumption. This variant is for buffers registered via
...
{
	const uint64_t pagesize       = registry->config->pagesize;
	const uint64_t pagesize_shift = registry->config->pagesize_shift;
 ...
 * a mapping registry.
 *
 * Each entry is resolved page-by-page through the registry's phys_lut. This
 * variant is for iovecs registered via cudamem_mapping; heap-allocated
					   ...
					   struct nvme_command *cmd)
{
	...
	return nvme_request_prep_command_prps_contig_cuda_mapped(request, registry, dbuf,
								 dbuf_nbytes, cmd);
}
					...
					struct nvme_command *cmd)
{
	...
	return nvme_request_prep_command_prps_iov_cuda_mapped(request, registry, dvec, dvec_cnt,
							      cmd);
}
